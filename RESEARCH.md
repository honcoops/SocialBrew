# Social Brew Website — Research & Feature List

## About Social Brew

- **Name:** Social Brew
- **Type:** Craft beer tasting bar (curates & serves, does not brew on-site)
- **Address:** 140 N Main St, Crossville, TN 38555
- **Phone:** (931) 456-2739
- **Email:** SocialBrew180@gmail.com
- **Hours:** Wed 3–9pm, Thu 3–10pm, Fri–Sat 3–11pm (Closed Sun–Tue)
- **Taps:** 10 rotating taps — Tennessee breweries (Calfkiller, Yee-Haw), ciders (Blake's Hard Cider), specialty/seasonal rotations focused on unique beers, ciders, sours, and porters
- **Cans/Bottles:** 60+ in cooler for on-site or retail purchase
- **Food:** No kitchen — BYOF, delivery welcome, occasional food trucks
- **Events:** Live music (Fridays), trivia, karaoke
- **Entertainment:** Darts, shuffleboard, TVs, live music stage
- **Vibe:** Relaxed downtown gathering spot. Board games, TVs, indoor/outdoor seating. Pet-friendly & family-friendly.
- **Building:** Renovated 1930s garage — former vehicle service shop, core part of identity
- **Has a pour wall** (self-serve tap wall)
- **Current Online Presence:**
  - **Facebook:** [facebook.com/SocialBrewCrossville](https://www.facebook.com/SocialBrewCrossville/) — 5,898 likes, 3,264 check-ins, 98% recommend (371 reviews)
  - **Instagram:** [@socialbrewcrossville](https://www.instagram.com/socialbrewcrossville/)
  - **Untappd:** [untappd.com/v/social-brew/3607999](https://untappd.com/v/social-brew/3607999) (unclaimed)
  - **Google:** 4.6/5 rating (98 reviews)
  - **Yelp:** 11 reviews
  - **TripAdvisor:** 93 reviews
  - **Explore Crossville:** Listed
  - **No dedicated website**

### Brand Identity (from Facebook/online)
- **Tagline:** "Pours warmth, flavor, and community in every glass"
- **Core positioning:** "A relaxed and welcoming craft beer tasting bar where old friends and new gather over locally curated brews and creative cocktails"
- **Brand pillars:** Community, warmth, local/craft focus, laid-back social atmosphere
- **Tone:** Casual, friendly, inclusive, down-to-earth
- Indoor and outdoor seating (outdoor patio with string lights)
- Downtown Crossville location — walkable, accessible
- Rustic/industrial aesthetic — exposed structure, repurposed garage space
- Intimate, cozy atmosphere

### Inferred Design Direction
| Element | Direction |
|---|---|
| **Color palette** | Warm, earthy tones — dark wood browns, amber/golden beer tones, deep greens or blacks. Rustic garage aesthetic. |
| **Typography** | Vintage/industrial character fonts for headings. Clean sans-serif for body. Chalkboard or hand-painted sign feel. |
| **Photography style** | Warm, string-lit ambiance. Craft beer closeups. People gathering casually. The patio. The tap/pour wall. |
| **Overall feel** | Warm, inviting, unpretentious. Neighborhood gathering spot, not a sleek lounge. "Come as you are" energy. |

---

## Competitive Landscape

### Direct Local Competitors

#### 1. Swing On Brewing Co (Crossville, TN)
- **Type:** Microbrewery with taproom
- **Location:** Potato Farm Rd, Crossville (countryside, not downtown)
- **Taps:** 11 beers on tap (brews their own)
- **Differentiators:** Grows own hops, Caddyshack theme, golf simulator
- **Website:** Has one — gives them online advantage
- **Social Brew advantage:** Downtown location, curated variety vs single-brewery

#### 2. Creekside Brewing Co (Crossville, TN)
- **Type:** Nano brewery with taproom
- **Location:** 11750 Hwy 127 S, Crossville
- **Note:** Seasonal operation (closes winters, reopens spring ~March)
- **Social Brew advantage:** Year-round operation, downtown location, wider selection

### Regional Competitors

#### 3. Cofer's Garage (Rockwood, TN — ~30 min away)
- **Type:** Beer garden & events venue (curates, doesn't brew — same model as Social Brew)
- **Address:** 141 S Front St, Rockwood, TN 37854
- **Hours:** Thu–Sat only
- **Website:** cofersgarage.com — well-designed with dark charcoal tones, Fjalla One headings
- **Features:**
  - Events calendar
  - "Cup Club" loyalty program
  - Mobile beer trailer (Cofer's 2.0)
  - Cigars & Guitars event series
  - FAQ page
  - Reviews page
  - Crew/team page
- **Design:** Dark charcoal backgrounds, warm taupe accents, sticky header, mobile-responsive
- **Tagline:** "Rockwood's Favorite Beer Garden & Events Venue"
- **Vibe:** Family-friendly, inclusive ("EVERYONE is welcome!"), has 3 spaces (Service Dept, Main Garage, Beer Garden)
- **What they do well:** Strong website, clear branding, events-driven, loyalty program
- **Social Brew opportunity:** More days open, downtown walkability, larger tap selection, pour wall

#### 4. Calfkiller Brewing Company (Sparta, TN — ~35 min away)
- **Type:** Craft brewery (brews on-site)
- **Address:** 1839 Blue Springs Rd, Sparta, TN
- **Website:** calfkillerbeer.com (basic/dated design)
- **Beers:** 20 beer rotation, 5 staples always on tap + seasonals
- **Notable brews:** Grassroots Pale Ale, Deadhorse Porter, Smokey Treat Smoked Barley Ale
- **Features:**
  - Beer list page
  - Online merch store (Storenvy)
  - Brewery tours (Saturdays by appointment)
  - Food truck on-site
  - Gift shop with growlers
- **Motto:** "Unparalleled Drinkability, Paradoxically Balanced by Uncompromised Complexity"
- **What they do well:** Known brand (Social Brew even carries their beer), established brewery cred
- **Social Brew opportunity:** Better website design, more modern UX, events focus, curated multi-brewery selection

### Additional Area Competitors
- **Lilly Pad Hopyard Brewery** — Small batch, near Obed Wild & Scenic River National Park

---

## Competitive Analysis Summary

| Feature | Social Brew | Cofer's Garage | Calfkiller | Swing On |
|---|---|---|---|---|
| Website | ❌ None | ✅ Good | ⚠️ Basic/dated | ✅ Has one |
| Taps | 10 rotating | Unknown | 20 (own brews) | 11 (own brews) |
| Cans/Bottles | 60+ | Unknown | Growlers | Unknown |
| Pour Wall | ✅ | ❌ | ❌ | ❌ |
| Food | BYOF/trucks | Food available | Food truck | Unknown |
| Events | Music, trivia, karaoke | Music, games, cigars | Tours | Golf sim |
| Loyalty Program | ❌ | ✅ Cup Club | ❌ | ❌ |
| Online Store | ❌ | ❌ | ✅ Storenvy | ❌ |
| Location | Downtown | Small town | Rural | Countryside |
| Days Open | 4 days | 3 days | Varies | Varies |

### Social Brew's Key Differentiators
1. **Pour wall** — unique self-serve experience, no competitor has this
2. **Downtown Crossville** — walkable, central location
3. **Curated variety** — 10 taps from many breweries vs. single-brewery taprooms
4. **60+ cans/bottles** — largest packaged selection in area
5. **Community hub** — board games, multiple event types, indoor/outdoor seating

---

## Website Feature List (Prioritized)

### Phase 1 — Launch MVP

| # | Feature | Priority | Notes |
|---|---|---|---|
| 1 | **Age gate modal** | Must-have | Simple 21+ overlay, non-SEO-blocking |
| 2 | **Homepage** | Must-have | Hero image, hours, address, tagline, CTAs |
| 3 | **Tap list page** | Must-have | JSON-driven; name, style, ABV, description, new/seasonal badges |
| 4 | **Can/bottle list** | Must-have | JSON-driven packaged beer inventory by category |
| 5 | **Events page** | Must-have | Upcoming live music, trivia, karaoke with dates |
| 6 | **About page** | Must-have | Story, philosophy, community focus, the pour wall |
| 7 | **Contact/Visit page** | Must-have | Google Map, address, phone, hours, parking, pet/family-friendly |
| 8 | **Responsive nav + footer** | Must-have | Sticky header, mobile hamburger menu, footer with essentials |
| 9 | **Mobile-first responsive** | Must-have | Touch-friendly, fast-loading, no PDF menus |
| 10 | **Local SEO** | Must-have | Schema.org markup, meta tags, Google Business integration |
| 11 | **Social media links** | Must-have | Facebook, Untappd links in nav/footer |
| 12 | **Brand-consistent design** | Must-have | Colors/style matching Facebook branding |

### Phase 2 — Enhancements

| # | Feature | Priority | Notes |
|---|---|---|---|
| 13 | **Newsletter signup** | Nice-to-have | Email capture, Mailchimp or similar |
| 14 | **Photo gallery** | Nice-to-have | Taproom, pour wall, events, atmosphere |
| 15 | **Social media feed** | Nice-to-have | Embedded Facebook/Instagram feed |
| 16 | **Food truck schedule** | Nice-to-have | When and which trucks are coming |
| 17 | **Blog/news** | Nice-to-have | New tap announcements, event recaps |
| 18 | **Pour wall explainer** | Nice-to-have | How it works, pricing, unique selling point |

### Phase 3 — Growth & Differentiation

| # | Feature | Priority | Notes |
|---|---|---|---|
| 19 | **Online can/bottle ordering** | Future | Order for pickup |
| 20 | **Mug club / loyalty** | Future | Compete with Cofer's Cup Club |
| 21 | **Merch shop** | Future | Branded items, gift cards |
| 22 | **Private event booking** | Future | Inquiry form for parties |
| 23 | **Analytics** | Future | GA4, event tracking |
| 24 | **Beer education section** | Future | Style guides, pour wall tips, food pairings |

---

## Tech Stack Decision

**Framework:** Astro
- Ships zero JS by default — fast, static pages
- JSON data files for tap list management (edit + redeploy)
- Interactive "islands" only where needed (age gate, tap list filters)
- Deploys to any static host (Netlify, Vercel, Cloudflare Pages, GitHub Pages)
- Component-based, clean, maintainable

**Tap List Management:** JSON data files
- `/src/data/taps.json` — current on-tap beers
- `/src/data/cans.json` — packaged beer inventory
- `/src/data/events.json` — upcoming events
- Easy to update without touching code

**Hosting:** Static (Netlify, Vercel, or Cloudflare Pages — all free tier)

---

## Design Direction

Based on Social Brew's brand personality and competitive positioning:
- **Warm, inviting palette** — earthy tones reflecting the relaxed downtown vibe
- **Clean, modern layout** — beat Calfkiller's dated site, match/exceed Cofer's quality
- **Photography-forward** — placeholder hero sections ready for real venue photos
- **Community-centered messaging** — "where old friends and new gather"
- **Highlight the pour wall** — unique differentiator, should be prominent

---

## Sources

- [Cofer's Garage Website](https://cofersgarage.com/)
- [Calfkiller Brewing Company](http://www.calfkillerbeer.com/)
- [Social Brew on Facebook](https://www.facebook.com/SocialBrewCrossville/)
- [Social Brew on Untappd](https://untappd.com/v/social-brew/3607999)
- [Social Brew on Explore Crossville](https://explorecrossville.com/listing/social-brew/)
- [Social Brew on Yelp](https://www.yelp.com/biz/social-brew-crossville)
- [Social Brew on TripAdvisor](https://www.tripadvisor.com/Attraction_Review-g54986-d10413704-Reviews-Social_Brew-Crossville_Tennessee.html)
- [Cofer's Garage on Yelp](https://www.yelp.com/biz/cofer-s-garage-rockwood)
- [Calfkiller on BeerAdvocate](https://www.beeradvocate.com/beer/profile/25232/)
- [Crossville Breweries on BeerMenus](https://www.beermenus.com/cities/1462-crossville-tn/nearby/breweries)
